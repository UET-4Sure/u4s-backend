...
export class Migrations1748762926592 implements MigrationInterface {
    name = 'Migrations1748762926592'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`liquidity_events\` (\`id\` varchar(36) NOT NULL, \`type\` enum ('MINT', 'BURN') NOT NULL, \`liquidity_amount\` decimal(38,18) NOT NULL, \`amount0\` decimal(38,18) NOT NULL, \`amount1\` decimal(38,18) NOT NULL, \`tx_hash\` varchar(66) NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`position_id\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`pool_metrics\` (\`id\` varchar(36) NOT NULL, \`bucket_start\` timestamp NOT NULL, \`tvl_usd\` decimal(38,18) NOT NULL DEFAULT '0', \`volume_24h_usd\` decimal(38,18) NOT NULL DEFAULT '0', \`fees_24h_usd\` decimal(38,18) NOT NULL DEFAULT '0', \`apr_for_lps\` decimal(10,4) NOT NULL DEFAULT '0', \`price_ratio\` decimal(38,18) NOT NULL DEFAULT '0', \`liquidity\` decimal(38,18) NOT NULL DEFAULT '0', \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`pool_id\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`pools\` (\`id\` varchar(36) NOT NULL, \`address\` varchar(42) NOT NULL, \`fee_tier\` int NOT NULL, \`tick_spacing\` int NOT NULL, \`hook_address\` varchar(42) NULL, \`initialized\` tinyint NOT NULL DEFAULT 0, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`token0_id\` varchar(36) NULL, \`token1_id\` varchar(36) NULL, UNIQUE INDEX \`IDX_8f9d6a1e9ca7c169ba22b77d0e\` (\`address\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`token_price\` (\`id\` varchar(36) NOT NULL, \`priceUsd\` decimal(38,18) NOT NULL, \`timestamp\` timestamp NOT NULL, \`token_id\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`token_price\` ADD CONSTRAINT \`FK_cd060cfc0599a02dc0700e974ad\` FOREIGN KEY (\`token_id\`) REFERENCES \`tokens\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`token_price\` DROP FOREIGN KEY \`FK_cd060cfc0599a02dc0700e974ad\``);
        ...
        await queryRunner.query(`DROP INDEX \`IDX_8f9d6a1e9ca7c169ba22b77d0e\` ON \`pools\``);
